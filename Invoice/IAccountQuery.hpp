...
{
public:
	auto operator()(const Year& y) const { return get(y); }
	auto Name(const Year& y) const { return name(y); };
private:
	...
	virtual std::string name(const Year& y) const = 0;
};

class CostQuery: public IAccountQuery
{
private:
...
class ExtraCostQuery: public IAccountQuery
{
private:
	virtual std::vector<FactoryUnitContainer<FactoryUnitContainer<FactoryUnit<std::string,FactoryUnit<std::string, std::string>>>>> get(const Year& y) const
	{
