...
#define CALCULATORRESULT_HPP
	...
	template<typename T, class D, typename L, typename R=L>
	decltype(auto) operator-(const Result<D,L,R,T>& r, const T& t) { return Result<Subtraction,Result<D,L,R,T>, T, decltype(r.Get() - t)>(r,t,r.Get() - t); }
	
	template<typename T, class D, typename L, typename R=L>
	decltype(auto) operator-(const T& t, const Result<D,L,R,T>& r) { return Result<Subtraction,T,Result<D,L,R,T>, decltype(r.Get() - t)>(t,r,t - r.Get()); }
