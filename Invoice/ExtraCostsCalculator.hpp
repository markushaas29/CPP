...
private:
    ...
    auto calcCosts(auto stageMatrix, std::shared_ptr<Factory<IToken>> tokenFactory,std::shared_ptr<Factory<IElement>> elementFactory,std::shared_ptr<Factory<BaseVisitor>> visitorFactory, const std::string& path, const HtmlBuilder<German>& f, const Year& y) const
    {
		...
		inv.exec();
		auto inv2 = Invoice(MatrixFormatter(address).Lines(), Header{"Mietvertrag"}, Text{"ABC"}, Quantity<Sum>{500}, std::to_string(S::Index)+"_"+"Mietvertrag");
		inv2.exec();
//		auto inv3 = Invoice(MatrixFormatter(address).Lines(), Header{"Widerruf"}, Text{"Hiermit lege ich Widerruf auf ihr Schreiben vom 4.1.2026 ein"}, Quantity<Sum>{500}, "Widerruf");
//		inv3.exec();
