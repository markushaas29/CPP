...
namespace DateTimes
{
	...
	public:
		...
		Date(DateTimes::Day d, DateTimes::Month m,DateTimes::Year y): 
		...
		Date(const std::string& s, const TupleType& t): Date(std::get<DateTimes::Day>(t).Value(),  std::get<DateTimes::Month>(t).Value(),  std::get<DateTimes::Year>(t).Value()) { };
...
std::istream& operator>>(std::istream& is, DateTimes::Date& d)
{	
	std::string s;
	is>>s;
	d=DateTimes::Date{d};
