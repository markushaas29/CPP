...
class Red: public IElementDecorator
{
	inline static constexpr const char TypeIdentifier[] = "Red";
    inline static constexpr Literal TypeId{TypeIdentifier};
    template<typename U> using IsT =  Is<U,TypeId>;
private:
	virtual std::string decorate(const std::string s) const { return "<r>"+s+"<r>"; }
};
