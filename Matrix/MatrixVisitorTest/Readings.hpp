...
class Readings
{
	...
	//		auto mp3 = m22_23.Match(imatcher).Parse(matcher);
			...
			auto d1 = std::make_shared<Quantity<Scalar>>((*accV(0,2)).To<Quantity<Volume>>() / (*accV()).To<Quantity<Volume>>());
			auto d2 = std::make_shared<Quantity<Scalar>>((*accV(2,4)).To<Quantity<Volume>>() / (*accV()).To<Quantity<Volume>>());
			auto d3 = std::make_shared<Quantity<Scalar>>((*accV(4,6)).To<Quantity<Volume>>() / (*accV()).To<Quantity<Volume>>());
