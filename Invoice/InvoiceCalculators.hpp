...
//        heads->push_back(std::make_unique<StyleElement>(std::move(styleVec)));
		...
		div0->Add(Html<Td>(Date::Today(),std::make_unique<Css<Style<FontWeight,Bold>>>(),"Date", "DateId"));
		outs->push_back(std::move(div0));
		...
		div1->Add(mf.Html(std::make_unique<HtmlElement<Caption, Header>>(Header("Payments")),nullptr,"Sums", "Payments"));
