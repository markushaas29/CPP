...
template<class M, class Expr> 
inline decltype(auto) operator*(const Scalar<M>& s, std::shared_ptr<BinaryExpression<Expr>> expr)
{
	return std::make_shared<BinaryExpression<MultiplicationExpression<Scalar<M>,BinaryExpression<Expr>>>>(s, *expr);
}
//-------------------------------------------------------------------------------------------------------------
