...
protected:
	std::unique_ptr<IResult<Quantity<Unit<1>>, Matrix<2, MatrixDescriptor<2,std::shared_ptr<IElement>>>>, std::default_delete<IResult<Quantity<Unit<1>>, Matrix<2, MatrixDescriptor<2, std::shared_ptr<IElement>>>>>> getResult(const Year& y, const HtmlBuilder<German>& f, const std::vector<FactoryUnitContainer<FactoryUnitContainer<FactoryUnit<std::string,FactoryUnit<std::string, std::string>>>>>& allFactoryUnits, const std::string& n) const
	{
		...
		return (*all)(parsedAccountMatrix);
	}
...
public:
	...
	auto Result(const Year& y, const HtmlBuilder<German>& f) const { return getTokens2(y,f); }
private:
	...
	{
		std::vector<FactoryUnitContainer<FactoryUnitContainer<FactoryUnit<std::string,FactoryUnit<std::string, std::string>>>>> allFactoryUnits = getUnits(y);
		
		return Base::exec(y,f, (*q)(y),q->Name(y));
	}
	std::unique_ptr<IResult<Quantity<Unit<1>>, Matrix<2, MatrixDescriptor<2,std::shared_ptr<IElement>>>>, std::default_delete<IResult<Quantity<Unit<1>>, Matrix<2, MatrixDescriptor<2, std::shared_ptr<IElement>>>>>> getTokens2(const Year& y, const HtmlBuilder<German>& f, std::unique_ptr<IAccountQuery> q = std::make_unique<ExtraCostQuery>()) const
	{
		std::vector<FactoryUnitContainer<FactoryUnitContainer<FactoryUnit<std::string,FactoryUnit<std::string, std::string>>>>> allFactoryUnits = getUnits(y);
		
		return Base::getResult(y,f, (*q)(y),q->Name(y));
	}
	std::vector<FactoryUnitContainer<FactoryUnitContainer<FactoryUnit<std::string,FactoryUnit<std::string, std::string>>>>> getUnits(const Year& y) const
	{
