...
public:
	...
	decltype(auto) operator[] (const std::string& s) const { return access->matrix(s,this); }
	decltype(auto) operator[] (std::shared_ptr<IElement> i) const { return (*this[i->Data()]); }
	...
	decltype(auto) Cols(auto... i) const { return access->cols(this, i...); }
	...
	decltype(auto) RowSum() const { return MC<Type>::rowSum(*this); }
  	...
  	decltype(auto) operator^(const auto v)	const { return MC<Type>::transform(*this);   	}
	template<size_t ND>
  	decltype(auto) Transform(auto... I)	const { return MC<Type>::template transformDim<ND>(*this, I...);   	}
...
private:
	...
	template<typename> friend class MatrixIO;
