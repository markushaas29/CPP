...
public:
	...
	decltype(auto) Sub(std::array<size_t,2> rowSpan, std::array<size_t,2> colSpan) const { return access->sub(rowSpan,colSpan, this); }
	...
	decltype(auto) Calc(Op op) { return MC<Type>::calc(op, elements->cbegin(), elements->cend(), descriptor); }
	...
	decltype(auto) RowSum() { return MC<Type>::rowSum(*this); }
  	decltype(auto) operator+(const auto& v)	{ return MC<Type>::apply(*this,ElementAdd<ElementType,decltype(v)>{Constant<decltype(v)>(v)},v);  }
  	decltype(auto) operator-(const auto& v)	{ return MC<Type>::apply(*this,ElementSub<ElementType,decltype(v)>{Constant<decltype(v)>(v)},v);  	}
  	decltype(auto) operator*(const auto& v)	{ return MC<Type>::apply(*this,ElementMul<ElementType,decltype(v)>{Constant<decltype(v)>(v)},v);  	}
  	decltype(auto) operator/(const auto& v)	{ return MC<Type>::apply(*this,ElementDiv<ElementType,decltype(v)>{Constant<decltype(v)>(v)},v);   	}
